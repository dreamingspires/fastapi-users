...
async def test_app_client_factory(
    ...
    async def app_factory(requires_verification):
        ...
        mock_auth_router = get_auth_router(mock_authentication, mock_user_db, authenticator, requires_verification=requires_verification)
        mock_bis_auth_router = get_auth_router(
            mock_authentication_bis, mock_user_db, authenticator, requires_verification= requires_verification
        )
...
@pytest.mark.asyncio
class TestLogin:
    async def test_empty_body(self, path, test_app_client_factory: Callable[[Any], httpx.AsyncClient], requires_verification):
    ...
    async def test_missing_username(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
        data = {"password": "guinevere"}
        response = await test_app_client.post(path, data=data)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    async def test_missing_password(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
        data = {"username": "king.arthur@camelot.bt"}
        response = await test_app_client.post(path, data=data)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    async def test_not_existing_user(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
    ...
    async def test_wrong_password(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
    ...
    async def test_valid_credentials_unverified(
        self, path, email, test_app_client_factory: httpx.AsyncClient, user: UserDB, requires_verification
    ):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
    ...
    @pytest.mark.parametrize(
        "email", ["lake.lady@camelot.bt", "Lake.Lady@camelot.bt"]
    )
    async def test_valid_credentials_verified(
        self, path, email, test_app_client_factory: httpx.AsyncClient, verified_user: UserDB, requires_verification
    ):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
    ...
    async def test_inactive_user(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
...
@pytest.mark.asyncio
class TestLogout:
    async def test_missing_token(self, path, test_app_client_factory: httpx.AsyncClient, requires_verification):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
        response = await test_app_client.post(path)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    async def test_valid_credentials_unverified(
        self, mocker, path, test_app_client_factory: httpx.AsyncClient, requires_verification, user: UserDB, 
    ):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
    ...
    async def test_valid_credentials_verified(
        self, mocker, path, test_app_client_factory: httpx.AsyncClient, requires_verification, verified_user: UserDB, 
    ):
        test_app_client = await test_app_client_factory(requires_verification).__anext__()
        response = await test_app_client.post(
